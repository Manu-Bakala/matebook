...
class ConversationRepository extends ServiceEntityRepository
{
    ...
    */
    ...
    public function findConversationByUser(int $userId)
    {
        $qb = $this->createQueryBuilder('c');
        $qb
            ->select('otherUser.id', 'otherUser.firstName','otherUser.lastName', 'c.id as conversationId', 'lm.content', 'lm.createdAt')
            ->innerJoin('c.participants', 'p', ExprJoin::WITH, $qb->expr()->neq('p.user', ':user' ))
            ->innerJoin('c.participants', 'me', ExprJoin::WITH, $qb->expr()->eq('me.user', ':user'))
